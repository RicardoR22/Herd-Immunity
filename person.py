...
class Person(object):
    ...
    def did_survive_infection(self):
        ...
        survive_chance = random.uniform(0,1)
        if self.infection != None:
            if survive_chance < self.infection.mortality_rate:
                self.is_alive = False
                return False
            else:
...
def test_person():
        person = Person(583, False)
        assert person._id is 583
...
def test_did_survive_infection():
        tuberculosis = Virus("tuberculosis", 0.67, 0.3)
        person = Person(823, False, tuberculosis)
